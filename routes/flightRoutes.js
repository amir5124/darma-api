...
router.get('/generate-ticket/:bookingCode', async (req, res) => {
    try {
        const pdfBuffer = await generateTicketBuffer(req.params.bookingCode);
...
async function generateTicketBuffer(bookingCode) {
    // 1. Ambil data dari database
    const [rows] = await db.execute("SELECT * FROM bookings WHERE booking_code = ?", [bookingCode]);
    ...
    // --- HELPER & MAPPING (Sama seperti kode Anda) ---
    const calculateDuration = (depart, arrival) => {
        if (!depart || !arrival) return '--';
        const start = new Date(depart);
        const end = new Date(arrival);
        const diffMs = end - start;
        return `${Math.floor(diffMs / 3600000)}j ${Math.round(((diffMs % 3600000) / 60000))}m`;
    };
    ...
    const renderFlightSection = (flightSegments, titleLabel, isReturn = false) => {
        ...
        return flightSegments.map((f) => {
            ...
            return `
            <div class="flight-box">
                <div class="flight-header">${titleLabel} - ${dateObj.toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</div>
                <div class="flight-content">
                    <div class="airline-info">
                        <div class="airline-name">${airlineNames[booking.airline_id] || booking.airline_id}</div>
                        <div class="flight-number">${booking.airline_id} ${f.flightNumber}</div>
                    ...
                    <div class="route-display">
                        ...
                        <div class="path-line">
                            <div class="duration">${calculateDuration(departTime, arrivalTime)}</div>
    ...
    const paxRows = passengers.map((p, pIdx) => {
        const isInfant = p.type === 'Infant' || parseInt(p.type) === 2;
        const typeLabel = isInfant ? 'Infant<small>Bayi</small>' : (p.type === 'Child' || parseInt(p.type) === 1 ? 'Child<small>Anak</small>' : 'Adult<small>Dewasa</small>');
        const adPergi = payload.paxDetails?.[pIdx]?.addOns?.[0] || null;
        const bagInfo = isInfant ? '-' : (adPergi?.baggageString ? (baggageMap[adPergi.baggageString] || adPergi.baggageString) : (defaultBaggage[booking.airline_id] || "0kg"));

        return `<tr>
            <td style="text-align:center">${pIdx + 1}</td>
            <td><b>${p.title} ${p.firstName} ${p.lastName}</b></td>
            <td>${typeLabel}</td>
            <td style="text-align:center">${adPergi?.seat || '-'}</td>
            <td style="text-align:center">${bagInfo}</td>
            <td style="font-size:8.5px;">${adPergi?.meals?.map(m => mealMap[m] || m).join(', ') || '-'}</td>
        </tr>`;
    }).join('');

    const htmlContent = `<html><head><style>/* CSS ANDA DISINI */ body { font-family: Arial; font-size: 10px; } .flight-box { border: 1px solid #24b3ae; margin-bottom: 10px; } .flight-header { background: #24b3ae; color: white; padding: 5px; } .flight-content { display: flex; padding: 10px; } .airline-info { width: 30%; border-right: 1px solid #eee; } .route-display { width: 70%; display: flex; justify-content: space-between; padding-left: 10px; } .time-text { font-size: 14px; font-weight: bold; } .section-title { background: #019387; color: white; padding: 5px; } table { width: 100%; border-collapse: collapse; } th, td { border-bottom: 1px solid #eee; padding: 8px; text-align: left; }</style></head><body><div style="padding: 20px;">
        <table width="100%"><tr><td><img src="https://res.cloudinary.com/dgsdmgcc7/image/upload/v1768877917/WhatsApp_Image_2026-01-20_at_09.45.43-removebg-preview_lqkgrw.png" height="40"></td><td align="right"><img src="${qrDataUrl}" width="60"><br><b>${response.bookingCodeAirline || booking.booking_code}</b></td></tr></table>
        ${renderFlightSection(response.flightDeparts || response.schDeparts, 'Departure')}
        <div class="section-title">Passenger Detail</div>
        <table><thead><tr><th>No</th><th>Name</th><th>Type</th><th>Seat</th><th>Bag</th><th>Meal</th></tr></thead><tbody>${paxRows}</tbody></table>
    </div></body></html>`;
