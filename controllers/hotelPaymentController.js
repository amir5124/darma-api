        ...
        try {
            ...
            await connection.query(
                `UPDATE hotel_payments SET 
                    ...
                    amount = ?
                 WHERE booking_id = ?`,
                [partner_reff, method === 'VA' ? `VA-${bankName}` : 'QRIS', vaNumber, qrisImage, feeAdmin, amount, booking_id]
            );

            // 5. Kirim Email Instruksi (Template disesuaikan Hotel)
            const emailHtml = `
            ...
            sendBookingEmail(customer_email, `Bayar Hotel - ${b.reservation_no}`, emailHtml).catch(e => console.error(e));
